...
# операторов: сложение (__add__()), вычитание (__sub__()), умножение (__mul__()), деление
# (__truediv__()). Данные методы должны применяться только к клеткам и выполнять увеличение,
...
class Cell:
    ...
    def __str__(self):
        return f'Результат операции {self.a * "*"}'

    def __add__(self, other):
        return Cell(self.a + other.a)

    def __sub__(self, other):
        if int(self.a - other.a) > 0:
            return Cell(int(self.a - other.a))
        else:
            return f'Результат отрицательный. Операция невозможна'
    ...
    def make_order(self, cells_in_row):
        row = ''
        for _ in range(int(self.a / cells_in_row)):
            row += f'{"*" * cells_in_row}\n'
        row += f'{"*" * (self.a % cells_in_row)}'
        return row
...
print((cells1 * cells2).make_order(20))
print(cells1 / cells2)
